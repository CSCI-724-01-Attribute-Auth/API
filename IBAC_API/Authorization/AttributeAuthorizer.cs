...
namespace API.Authorization
{
    public class AttributeAuthorizer
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (clientId == null)
            {
                throw new InvalidOperationException("Client ID is missing from the token");
            }
        ...
        private string? GetClientIdFromToken(string token)
        {
            try
            {
                ...
                var clientIdClaim = jwtToken.Claims.FirstOrDefault(c => c.Type == "nameid");
                return clientIdClaim?.Value;
            }
            catch (Exception ex)
            {
                throw new InvalidOperationException("Failed to extract client ID from token", ex);
            }
        }
    }
}
